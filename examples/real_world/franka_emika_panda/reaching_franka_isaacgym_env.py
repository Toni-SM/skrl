...
class ReachingFrankaTask(VecTask):
    def __init__(self, cfg):
        ...
        if self._control_space == "cartesian":
            ...
            self.jacobian_end_effector = self.jacobian[
                :, self.rigid_body_dict_robot[self._end_effector_link] - 1, :, :7
            ]
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        robot_dof_stiffness = torch.tensor(
            [400, 400, 400, 400, 400, 400, 400, 1.0e6, 1.0e6], dtype=torch.float32, device=self.device
        )
        robot_dof_damping = torch.tensor(
        ...
        for i in range(self.num_envs):
            ...
            self.gym.set_rigid_body_color(env_ptr, target_actor, 0, gymapi.MESH_VISUAL, gymapi.Vec3(1.0, 0.0, 0.0))
    ...
    def init_data(self):
        self.robot_default_dof_pos = torch.tensor(
            np.radians([0, -45, 0, -135, 0, 90, 45, 0, 0]), device=self.device, dtype=torch.float32
        )
        self.robot_dof_targets = torch.zeros(
            (self.num_envs, self.num_robot_dofs), device=self.device, dtype=torch.float32
        )
    ...
    def reset_idx(self, env_ids):
        ...
        pos = torch.clamp(
            ...
            + 0.25 * (torch.rand((len(env_ids), self.num_robot_dofs), device=self.device) - 0.5),
            self.robot_dof_lower_limits,
            self.robot_dof_upper_limits,
        )
        pos[:, 7:] = 0
        ...
        self.gym.set_dof_position_target_tensor_indexed(
            self.sim, gymtorch.unwrap_tensor(self.robot_dof_targets), gymtorch.unwrap_tensor(indexes), len(env_ids)
        )

        self.gym.set_dof_state_tensor_indexed(
            self.sim, gymtorch.unwrap_tensor(self.dof_state), gymtorch.unwrap_tensor(indexes), len(env_ids)
        )
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            self.sim, gymtorch.unwrap_tensor(self.root_state), gymtorch.unwrap_tensor(indexes), len(env_ids)
        )
    ...
    def pre_physics_step(self, actions):
        ...
        if self._control_space == "joint":
            ...
            delta_dof_pos = isaacgym_utils.ik(
                jacobian_end_effector=self.jacobian_end_effector,
        ...
        self.robot_dof_targets[:, :7] = torch.clamp(
            targets, self.robot_dof_lower_limits[:7], self.robot_dof_upper_limits[:7]
        )
