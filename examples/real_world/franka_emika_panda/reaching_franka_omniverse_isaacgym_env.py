...
class ReachingFrankaTask(RLTask):
    ...
    def set_up_scene(self, scene) -> None:
        ...
        if self._control_space == "cartesian":
            self._hands = RigidPrimView(
                prim_paths_expr="/World/envs/.*/robot/panda_hand", name="hand_view", reset_xform_properties=False
            )
    ...
    def get_target(self):
        target = DynamicSphere(
            prim_path=self.default_zero_env_path + "/target", name="target", radius=0.025, color=torch.tensor([1, 0, 0])
        )
        self._sim_config.apply_articulation_settings(
            "target", get_prim_at_path(target.prim_path), self._sim_config.parse_actor_config("target")
        )
        target.set_collision_enabled(False)

    def init_data(self) -> None:
        self.robot_default_dof_pos = torch.tensor(
            np.radians([0, -45, 0, -135, 0, 90, 45, 0, 0]), device=self._device, dtype=torch.float32
        )
        self.actions = torch.zeros((self._num_envs, self.num_actions), device=self._device)

        if self._control_space == "cartesian":
            self.jacobians = torch.zeros((self._num_envs, 10, 6, 9), device=self._device)
            self.hand_pos, self.hand_rot = torch.zeros((self._num_envs, 3), device=self._device), torch.zeros(
    ...
    def pre_physics_step(self, actions) -> None:
        ...
        if self._control_space == "joint":
            targets = (
                ...
                + self.robot_dof_speed_scales[:7] * self.dt * self.actions * self._action_scale
            )

        elif self._control_space == "cartesian":
            goal_position = self.hand_pos + actions / 100.0
            delta_dof_pos = omniverse_isaacgym_utils.ik(
                jacobian_end_effector=self.jacobians[:, 8 - 1, :, :7],  # franka hand index: 8
    ...
    def reset_idx(self, env_ids) -> None:
        ...
        pos = torch.clamp(
            ...
            + 0.25 * (torch.rand((len(env_ids), self.num_robot_dofs), device=self._device) - 0.5),
        ...
        # reset target
        pos = (torch.rand((len(env_ids), 3), device=self._device) - 0.5) * 2 * torch.tensor(
    ...
    def post_reset(self):
        ...
        self.robot_dof_targets = torch.zeros(
            (self._num_envs, self.num_robot_dofs), dtype=torch.float, device=self._device
        )
