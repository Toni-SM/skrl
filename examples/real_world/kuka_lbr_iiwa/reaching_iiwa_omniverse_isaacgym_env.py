...
class ReachingIiwaTask(RLTask):
    ...
    def init_data(self) -> None:
        self.robot_default_dof_pos = torch.tensor(
            np.radians([0, 0, 0, -90, 0, 90, 0]), device=self._device, dtype=torch.float32
        )
    ...
    def pre_physics_step(self, actions) -> None:
        ...
        if self._control_space == "joint":
            ...
            delta_dof_pos = omniverse_isaacgym_utils.ik(
                jacobian_end_effector=self.jacobians[:, 7 - 1, :, :7],  # iiwa_link_7 index: 7
    ...
    def reset_idx(self, env_ids) -> None:
        ...
        pos = (torch.rand((len(env_ids), 3), device=self._device) - 0.5) * 2 * torch.tensor(
            [0.10, 0.20, 0.20], device=self._device
