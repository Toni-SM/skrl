...
class ReachingIiwa(gym.Env):
    def __init__(self, control_space="joint"):
        ...
        self.pub_command_cartesian = self.node.create_publisher(
        ...
        client_joint_velocity_rel = self.node.create_client(
            libiiwa_msgs.srv.SetNumber, "/iiwa/set_desired_joint_velocity_rel"
        )
        client_joint_acceleration_rel = self.node.create_client(
            libiiwa_msgs.srv.SetNumber, "/iiwa/set_desired_joint_acceleration_rel"
        )
        client_joint_jerk_rel = self.node.create_client(libiiwa_msgs.srv.SetNumber, "/iiwa/set_desired_joint_jerk_rel")

        client_cartesian_velocity = self.node.create_client(
            libiiwa_msgs.srv.SetNumber, "/iiwa/set_desired_cartesian_velocity"
        )
        client_cartesian_acceleration = self.node.create_client(
            libiiwa_msgs.srv.SetNumber, "/iiwa/set_desired_cartesian_acceleration"
        )
    ...
    def step(self, action):
        ...
        # joint
        if self.control_space == "joint":
            joint_positions = self.robot_state["joint_position"] + (
                self.robot_dof_speed_scales * self.dt * action * self.action_scale
            )
            msg = sensor_msgs.msg.JointState()
