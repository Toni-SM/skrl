...
class Actor(GaussianMixin, Model):
    def __init__(
        ...
        self.linear_layer_3 = nn.Linear(64, self.num_actions)
        self.log_std_parameter = nn.Parameter(torch.zeros(self.num_actions))

    def compute(self, inputs, role):
        x = F.relu(self.linear_layer_1(inputs["observations"]))
        x = F.relu(self.linear_layer_2(x))
        return self.linear_layer_3(x), {"log_std": self.log_std_parameter}
...
class Critic(DeterministicMixin, Model):
    def __init__(self, observation_space, state_space, action_space, device):
        ...
        self.linear_layer_1 = nn.Linear(self.num_observations + self.num_actions, 64)
        self.linear_layer_2 = nn.Linear(64, 64)
...
# load the environment (note: the environment version may change depending on the dm-control version)
gym.register_envs(shimmy)  # unnecessary but prevents IDEs from complaining
task_name = "dm_control/acrobot-swingup_sparse"
