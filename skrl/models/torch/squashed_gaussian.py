...
class SquashedGaussianMixin:
    ...
    def act(
        ...
        gaussian_actions = self._distribution.rsample()

        # clip actions
        if self._clip_actions:
            gaussian_actions = torch.clamp(gaussian_actions, min=self._clip_actions_min, max=self._clip_actions_max)
        ...
        if should_log_prob:
            ...
            log_prob = self._distribution.log_prob(inputs.get("taken_actions", gaussian_actions))
            log_prob = sum_independent_dims(log_prob)
            # Squash correction
            log_prob -= torch.sum(torch.log(1 - squashed_actions**2 + 1e-6), dim=1)

        outputs["mean_actions"] = mean_actions
        return squashed_actions, log_prob, outputs
