...
class Trainer:
    ...
    def _pre_interaction(self, timestep: int, timesteps: int) -> None:
        ...
        if self._is_multi_agent:
            ...
            self._agents.pre_interaction(timestep=timestep, timesteps=timesteps)
    ...
    def _post_interaction(self, timestep: int, timesteps: int) -> None:
        ...
        if self._is_multi_agent:
            ...
            self._agents.post_interaction(timestep=timestep, timesteps=timesteps)
    ...
    def train(self, cfg: dict, env: Union[Environment, gym.Env], agents: Union[Agent, List[Agent]], agents_scope : List[int] = []) -> None:
        ...
        for timestep in range(self._current_learning_iteration, self._max_learning_iterations):
            ...
            if self._is_multi_agent:
                ...
                self._agents.record_transition(states, actions, rewards, next_states, dones, timestep, self._max_learning_iterations)
            ...
            # post-interaction
            self._post_interaction(timestep=timestep, timesteps=self._max_learning_iterations)
