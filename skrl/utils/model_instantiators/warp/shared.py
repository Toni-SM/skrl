    ...
    *,
    ...
    def get_return(class_name):
        if class_name.lower() == "deterministicmixin":
            ...
            return r'output, {"log_std": self.log_std_parameter.data}'
        raise ValueError(f"Unknown class: {class_name}")
    ...
    def get_extra(class_name, parameter, role, model):
        if class_name.lower() == "deterministicmixin":
            ...
            return f'self.log_std_parameter = nn.Parameter(wp.full(shape=({model["output"]["size"]},), value={initial_log_std}, dtype=wp.float32), requires_grad={not fixed_log_std})'
        raise ValueError(f"Unknown class: {class_name}")
