...
def test_env(capsys: pytest.CaptureFixture, num_states: int):
    ...
    for _ in range(2):
        ...
        state = env.state()
        assert isinstance(observation, wp.array) and observation.shape == (num_envs, 4)
...
def test_multi_agent_env(capsys: pytest.CaptureFixture, num_states: int):
    ...
    action = {f"agent_{i}": wp.ones((num_envs, i + 10)) for i in range(num_agents)}

    # check wrapper definition
    with pytest.raises(NotImplementedError):
        assert isinstance(wrap_env(None, "isaaclab-multi-agent"), IsaacLabMultiAgentWrapper)
    pytest.skip("Remove once implemented!")
    ...
    for _ in range(2):
        ...
        assert isinstance(info, Mapping)
        for i, agent in enumerate(possible_agents):
            assert isinstance(observation[agent], wp.array) and observation[agent].shape == (num_envs, i + 20)
        if num_states:
            assert isinstance(state, wp.array) and state.shape == (num_envs, num_states)
        else:
            assert state is None
        for _ in range(3):
            ...
            for i, agent in enumerate(possible_agents):
                assert isinstance(observation[agent], wp.array) and observation[agent].shape == (num_envs, i + 20)
                assert isinstance(reward[agent], wp.array) and reward[agent].shape == (num_envs, 1)
                assert isinstance(terminated[agent], wp.array) and terminated[agent].shape == (num_envs, 1)
                assert isinstance(truncated[agent], wp.array) and truncated[agent].shape == (num_envs, 1)
            if num_states:
                assert isinstance(state, wp.array) and state.shape == (num_envs, num_states)
            else:
                assert state is None
