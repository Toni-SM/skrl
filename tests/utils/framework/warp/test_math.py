...
def test_std(capsys, ndim, dtype, shape):
    ...
    assert np.allclose(value.numpy().item(), np.std(sample, ddof=0), rtol=1e-05, atol=1e-08)

    value = warp_utils.std(array, correction=1)
    assert np.allclose(value.numpy().item(), np.std(sample, ddof=1), rtol=1e-05, atol=1e-08, equal_nan=True)
