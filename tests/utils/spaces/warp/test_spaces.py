...
def test_compute_space_limits(capsys):
    ...
    assert (low.numpy() == expected_low).all(), f"Expected {expected_low.tolist()}, got {low.numpy().tolist()}"
    assert (high.numpy() == expected_high).all(), f"Expected {expected_high.tolist()}, got {high.numpy().tolist()}"
    ...
    assert low is None and np.isinf(high.numpy()).all()
    low, high = compute_space_limits(space, none_if_unbounded="above")
    assert np.isinf(low.numpy()).all() and high is None
...
def test_tensorize_space(capsys, space: gymnasium.spaces.Space):
    def check_tensorized_space(s, x, n):
        if isinstance(s, gymnasium.spaces.Box):
            assert isinstance(x, wp.array) and x.shape == (n, *s.shape)
        elif isinstance(s, gymnasium.spaces.Discrete):
            assert isinstance(x, wp.array) and x.shape == (n, 1)
        elif isinstance(s, gymnasium.spaces.MultiDiscrete):
            assert isinstance(x, wp.array) and x.shape == (n, *s.nvec.shape)
        elif isinstance(s, gymnasium.spaces.Dict):
